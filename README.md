...
[Check out the `add-authorization` branch](https://github.com/auth0-developer-hub/simplex_api/tree/add-authorization) to see authorization in action using Auth0.

[Check out the `add-rbac` branch](https://github.com/auth0-developer-hub/simplex_api/tree/add-rbac) to see authorization and Role-Based Access Control (RBAC) in action using Auth0.
...
- __X-XSS-Protection__
  ...
  But we still need to explicitly add the header to our custom [Auth0Middleware](simplex_api/middleware.py), otherwise Django will not add this header by default:
  ```python
...
- __HTTP Strict Transport Security (HSTS)__
  ...
  ```
  Note that this will only take effect for HTTPS requests. For HTTP we need to add this header to the [Auth0Middleware](simplex_api/middleware.py):
...
- __Cache-Control__
  ...
  We also need to add the `Expires = 0` header to the [Auth0Middleware](simplex_api/middleware.py), otherwise Django will set it with the actual date:
